  ...
  return (
    <header className="sticky top-0 z-50 px-3 pt-3 sm:px-6 sm:pt-4 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div
          className={`gradient-border bg-white/80 px-4 py-3 shadow-soft backdrop-blur-md transition-all sm:px-5 ${
            isOpen ? 'rounded-[1.75rem]' : 'rounded-full'
          }`}
        >
          <div className="flex items-center justify-between gap-4">
            <a className="min-w-0" href="#home" onClick={() => setIsOpen(false)}>
              <p className="truncate text-xs font-semibold uppercase tracking-[0.16em] text-brand-700 sm:text-sm sm:tracking-[0.25em]">
                Larry A. Dela Cruz Jr.
              </p>
              <p className="hidden truncate text-xs text-slate-500 sm:block">Virtual Assistant Portfolio</p>
            </a>

            <button
              type="button"
              className="inline-flex rounded-full border border-brand-100 p-2 text-brand-700 md:hidden"
