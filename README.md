...
- The contact form now includes a hidden Formspree `_gotcha` honeypot field, a short minimum submit delay, client-side filtering for suspicious URLs, inline field validation, and a reCAPTCHA-ready placeholder area for future upgrade. For stronger spam protection, also enable reCAPTCHA and `Restrict to Domain` in your Formspree dashboard.
